...
export const useDataBroker = () => {
  ...
  const searchDataSource = async (dataSource, searchParams) => {
    try {
      // Handle Searchbug APIs directly
      if (dataSource.name.startsWith('Searchbug')) {
        return await callSearchbugAPI(dataSource, searchParams)
      }
      
      // Handle web scraping APIs
      if (dataSource.api_endpoint === '/api/scrape-broker') {
        return await callScrapingAPI(dataSource, searchParams)
      }
      
      // Handle other direct API endpoints
  ...
  // Call Searchbug APIs
  const callSearchbugAPI = async (dataSource, searchParams) => {
    const { callSearchbugAPI } = await import('../api-services/searchbug')
    return await callSearchbugAPI(dataSource, searchParams)
  }
  
  // Call web scraping APIs
  const callScrapingAPI = async (dataSource, searchParams) => {
    const requestBody = {
      broker: {
    ...
    if (!response.ok) {
      console.warn(`Scraping API failed for ${dataSource.name}:`, response.statusText)
      return null
    }
