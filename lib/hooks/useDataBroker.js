...
export const useDataBroker = () => {
  ...
  const searchDataSource = async (dataSource, searchParams) => {
    try {
      // Handle internal API endpoints (our backend routes)
      if (dataSource.api_endpoint && dataSource.api_endpoint.startsWith('/api/')) {
        return await callInternalAPI(dataSource, searchParams)
      }
      
      // Handle external HTTP endpoints (legacy - should be avoided due to CORS)
      if (dataSource.api_endpoint && dataSource.api_endpoint.startsWith('http')) {
        console.warn(`External API call attempted for ${dataSource.name}, this may fail due to CORS`)
        return await callDirectAPI(dataSource, searchParams)
      }
      
      console.warn(`No API handler configured for ${dataSource.name}`)
      return null
    } catch (error) {
  ...
  // Call internal API endpoints (our backend)
  const callInternalAPI = async (dataSource, searchParams) => {
    const requestBody = {
    ...
    if (!response.ok) {
      console.warn(`Internal API call failed for ${dataSource.name}:`, response.statusText)
      return null
    }
    ...
    if (!data.success) {
      console.warn(`API returned error for ${dataSource.name}:`, data.error)
      return null
    }
    
    return {
      dataFound: data.data_found || [],
      description: data.description || 'Data found via API',
  ...
  // Call direct API endpoints (legacy - avoid due to CORS)
  const callDirectAPI = async (dataSource, searchParams) => {
    const requestBody = {
      query: {
        full_name: searchParams.fullName,
        phone: searchParams.phone,
    ...
    const response = await fetch(dataSource.api_endpoint, {
      ...
      headers: {
        ...
        'X-API-Source': dataSource.name
      },
      body: JSON.stringify(requestBody)
    })

    if (!response.ok) {
      console.warn(`Direct API call failed for ${dataSource.name}:`, response.statusText)
      return null
    }
...
Reference: Data Deletion Request - ${new Date().toISOString().split('T')[0]}
  ...
  // Load expanded broker list from comprehensive public broker database
  const loadExpandedBrokerList = async () => {
    try {
      setLoading(true)
      setOperationProgress({ current: 0, total: 3, message: 'Loading comprehensive public broker list...' })
      
      // Comprehensive list of major public-facing data broker and people-search sites
      const expandedBrokers = [
        // Tier 1: High-risk comprehensive brokers
        {
          name: 'MyLife',
          url: 'https://mylife.com',
          risk_level: 'high',
          description: 'Detailed personal profiles with reputation scores and background information',
          data_types: ['name', 'age', 'address', 'phone', 'email', 'reputation', 'background', 'social_media', 'relatives'],
          opt_out_url: 'https://www.mylife.com/opt-out/',
          contact_email: 'privacy@mylife.com',
          scan_priority: 1
        },
        {
          name: 'PeopleFindersFree',
          url: 'https://peoplefindersfree.com',
          risk_level: 'high',
          description: 'Free people search with comprehensive contact information',
          data_types: ['name', 'address', 'phone', 'relatives', 'criminal_records', 'court_records'],
          opt_out_url: 'https://www.peoplefindersfree.com/opt-out',
          contact_email: 'privacy@peoplefindersfree.com',
          scan_priority: 1
        },
        {
          ...
          data_types: ['name', 'criminal_records', 'court_records', 'address_history', 'phone', 'email', 'aliases'],
        ...
        },
        {
          ...
          url: 'https://pipl.com',
          risk_level: 'high',
          description: 'Professional people search engine with deep web data',
          data_types: ['name', 'email', 'phone', 'social_media', 'professional_info', 'education'],
        ...
        },
        {
          ...
          risk_level: 'high',
          description: 'Fast people search with detailed personal information',
          data_types: ['name', 'address', 'phone', 'age', 'relatives', 'email', 'social_media'],
          opt_out_url: 'https://www.fastpeoplesearch.com/removal',
          contact_email: 'privacy@fastpeoplesearch.com',
          scan_priority: 1
        },
        {
          name: 'ThatsThem',
          ...
          risk_level: 'high',
          description: 'Free people search with contact and background information',
          data_types: ['name', 'address', 'phone', 'email', 'age', 'relatives', 'neighbors'],
          opt_out_url: 'https://thatsthem.com/optout',
          contact_email: 'privacy@thatsthem.com',
          scan_priority: 1
        },
        
        // Tier 2: Medium-risk public record aggregators
        {
          name: 'Radaris',
          url: 'https://radaris.com',
          risk_level: 'medium',
          description: 'Public records aggregator with contact information',
          data_types: ['name', 'address', 'phone', 'age', 'relatives', 'education', 'employment'],
        ...
        },
        {
          ...
          risk_level: 'medium',
          description: 'Public records and contact information search',
          data_types: ['name', 'address', 'phone', 'relatives', 'criminal_records', 'property_records'],
        ...
        },
        {
          ...
          opt_out_url: 'https://www.ussearch.com/consumer/ala/landing.do?did=590',
          contact_email: 'privacy@ussearch.com',
          scan_priority: 2
        },
        {
          name: 'PublicRecordsNow',
          url: 'https://publicrecordsnow.com',
          risk_level: 'medium',
          description: 'Public records search and people finder',
          data_types: ['name', 'address', 'phone', 'criminal_records', 'court_records', 'property_records'],
          opt_out_url: 'https://publicrecordsnow.com/static/view/optout/',
          contact_email: 'privacy@publicrecordsnow.com',
          scan_priority: 2
        },
        {
          name: 'Addresses',
        ...
        },
        {
          ...
          risk_level: 'medium',
          description: 'Voter registration and public records search',
          data_types: ['name', 'address', 'age', 'political_affiliation', 'voting_history'],
          opt_out_url: 'https://voterrecords.com/faq',
          contact_email: 'privacy@voterrecords.com',
          scan_priority: 2
        },
        
        // Tier 3: Standard directory services
        {
          name: 'AnyWho',
          url: 'https://anywho.com',
          risk_level: 'low',
          description: 'Online phone book and people directory',
          data_types: ['name', 'address', 'phone'],
          opt_out_url: 'https://www.anywho.com/optout',
          contact_email: 'privacy@anywho.com',
          scan_priority: 3
        },
        {
          name: 'WhitePages',
        ...
        },
        {
          ...
          risk_level: 'low',
          description: 'Business and residential directory service',
          data_types: ['name', 'phone', 'address', 'business_info'],
          opt_out_url: 'https://corporate.thryv.com/privacy/',
          contact_email: 'privacy@yellowpages.com',
          scan_priority: 3
        },
        {
          ...
          risk_level: 'medium',
          description: 'Free people search with basic public records',
          data_types: ['name', 'address', 'phone', 'relatives', 'age_range'],
          opt_out_url: 'https://www.truepeoplesearch.com/removal',
          contact_email: 'privacy@truepeoplesearch.com',
          scan_priority: 2
        },
        
        // Additional specialized brokers
        {
          name: 'FamilyTreeNow',
          ...
          contact_email: 'privacy@familytreenow.com',
          scan_priority: 2
        },
        {
          name: 'CheckPeople',
          ...
          risk_level: 'high',
          description: 'People search and background check service',
          data_types: ['name', 'address', 'phone', 'email', 'criminal_records', 'court_records', 'social_media'],
          opt_out_url: 'https://www.checkpeople.com/opt-out',
          contact_email: 'privacy@checkpeople.com',
          scan_priority: 1
        },
        {
          name: 'Intelius',
          ...
          risk_level: 'high',
          description: 'Comprehensive background checks and people search',
          data_types: ['name', 'address', 'phone', 'email', 'criminal_records', 'court_records', 'property_records'],
      ...
      return { success: true, added: expandedBrokers.length, total_brokers: expandedBrokers.length }
    } catch (err) {
      setError(err.message)
      return { success: false, error: err.message }
    } finally {
  ...
  // Comprehensive public broker scanning
  const scanPublicBrokerSites = async (personalInfo, scanOptions = {}) => {
    ...
    try {
      setLoading(true)
      setOperationProgress({ current: 0, total: 0, message: 'Initializing comprehensive broker scan...' })
      
      // Debug: Log current dataSources state
      console.log('🔍 Current dataSources state:', {
        total: dataSources.length,
        active: dataSources.filter(ds => ds.is_active).length,
        sample: dataSources.slice(0, 3).map(ds => ({ name: ds.name, is_active: ds.is_active }))
      })

      // Ensure we have brokers to scan - load expanded list if needed
      let allBrokers = dataSources.filter(ds => ds.is_active)
      
      if (allBrokers.length === 0) {
        console.log('📋 No brokers found in dataSources, loading expanded broker list...')
        setOperationProgress({ current: 0, total: 0, message: 'Loading broker database...' })
        
        const loadResult = await loadExpandedBrokerList()
        console.log('📋 Load result:', loadResult)
        
        if (loadResult.success) {
          // Refresh allBrokers after loading
          allBrokers = dataSources.filter(ds => ds.is_active)
          console.log(`📋 After loading - found ${allBrokers.length} active brokers`)
        } else {
          console.error('❌ Failed to load expanded broker list:', loadResult.error)
        }
      }
      
      // If still no brokers, use fallback hardcoded list
      if (allBrokers.length === 0) {
        console.log('📋 Using fallback broker list for scanning')
        allBrokers = getFallbackBrokerList()
        console.log('📋 Fallback brokers:', allBrokers.map(b => ({ name: b.name, id: b.id })))
      }
      
      const scanBrokers = scanOptions.priority ? 
        allBrokers.filter(broker => (broker.scan_priority || broker.priority || 2) <= scanOptions.priority) : 
        allBrokers
      
      console.log(`📋 Final broker selection:`, {
        total: allBrokers.length,
        afterPriorityFilter: scanBrokers.length,
        priority: scanOptions.priority,
        brokerNames: scanBrokers.map(b => b.name).slice(0, 5) // First 5 names
      })

      setOperationProgress({ current: 0, total: scanBrokers.length, message: 'Starting automated searches across public broker sites...' })
      
      const scanResults = []
      const batchSize = scanOptions.batchSize || 5 // Process in batches to avoid overwhelming servers
      
      for (let i = 0; i < scanBrokers.length; i += batchSize) {
        const batch = scanBrokers.slice(i, i + batchSize)
        
        console.log(`📦 Processing batch ${Math.floor(i / batchSize) + 1}:`, {
          batchSize: batch.length,
          brokers: batch.map(b => ({ name: b?.name, hasName: !!b?.name, hasUrl: !!b?.url }))
        })
        
        // Process batch in parallel
        const batchPromises = batch.map(async (broker, batchIndex) => {
          const currentIndex = i + batchIndex + 1
          
          // Validate broker object before processing
          if (!broker) {
            console.error(`❌ Broker at index ${batchIndex} is undefined/null`)
            return {
              error: 'Broker object is undefined',
              dataFound: [],
              description: 'Invalid broker object'
            }
          }
          
          if (!broker.name) {
            console.error(`❌ Broker at index ${batchIndex} missing name:`, broker)
            return {
              error: 'Broker missing name field',
              dataFound: [],
              description: 'Invalid broker object - missing name'
            }
          }
          
          setOperationProgress({ 
            current: currentIndex, 
            total: scanBrokers.length, 
            message: `Scanning ${broker.name}...` 
          })
          
          return await scanSingleBroker(broker, personalInfo)
        })
        ...
        batchResults.forEach((result, batchIndex) => {
          ...
          if (result.status === 'fulfilled' && result.value) {
            scanResults.push({
              ...
              scan_method: result.value.scraped ? 'web_scraping' : 'api'
            })
          } else {
            scanResults.push({
              broker_id: broker.id,
        ...
        if (i + batchSize < scanBrokers.length) {
          await new Promise(resolve => setTimeout(resolve, 2000))
        }
      }
      ...
      const scanRecord = {
        user_id: user.id,
        scan_type: 'public_broker_comprehensive',
        personal_info: personalInfo,
        brokers_scanned: scanBrokers.length,
      ...
      // Save to search history with enhanced data
      await saveSearch({
        full_name: personalInfo.fullName,
        phone: personalInfo.phone,
        email: personalInfo.email,
        ...
        results: scanResults.map(r => ({
          ...
          url: dataSources.find(ds => ds.id === r.broker_id)?.url || '#',
          dataFound: r.dataFound || [],
          riskLevel: dataSources.find(ds => ds.id === r.broker_id)?.risk_level || 'medium',
          description: r.description || (r.error ? `Error: ${r.error}` : 'No data found'),
          scan_method: r.scan_method,
          scan_timestamp: r.scan_timestamp
        })),
        scan_type: 'comprehensive_public_brokers'
      })
      
      return {
        success: true,
        scan_record: scanRecord,
        total_scanned: scanBrokers.length,
  ...
  // Enhanced single broker scanning with priority handling
  const scanSingleBroker = async (broker, personalInfo) => {
    try {
      console.log(`🔍 Starting scan for broker:`, {
        name: broker.name,
        url: broker.url,
      ...
      // Ensure broker object has required fields
      if (!broker.name) {
        throw new Error('Broker object missing name field')
      }

      // Determine scan method based on broker configuration
      if (broker.api_endpoint && broker.api_endpoint.startsWith('/api/')) {
        console.log(`📡 Using API endpoint: ${broker.api_endpoint}`)
        return await callInternalAPI(broker, personalInfo)
      } else {
        console.log(`🌐 Using web scraping for: ${broker.name}`)
        return await scrapeDataBroker(broker, personalInfo)
      }
    } catch (error) {
      console.warn(`❌ Failed to scan ${broker.name}:`, error)
      return {
        error: error.message,
        dataFound: [],
  ...
  const scrapeDataBroker = async (broker, personalInfo) => {
    try {
      // Ensure proper parameter structure for the scrape-broker endpoint
      const searchParams = {
      ...
      // Construct broker object for API call
      const brokerPayload = {
        name: broker.name,
        url: broker.url,
        risk_level: broker.risk_level
      ...
      console.log('🔍 Calling scrape-broker API with payload:', {
        brokerName: broker.name,
        brokerPayload: brokerPayload,
        searchParams: {
          hasFullName: !!searchParams.fullName,
          hasPhone: !!searchParams.phone,
          hasEmail: !!searchParams.email,
          fullNameValue: searchParams.fullName
        },
      ...
      if (response.ok) {
        ...
        console.error(`❌ Scrape-broker API error for ${broker.name}:`, {
          status: response.status,
          statusText: response.statusText,
          error: errorData.error,
          brokerSent: brokerPayload,
        ...
        return {
          ...
          dataFound: [],
          riskLevel: broker.risk_level,
          description: `API Error: ${errorData.error}`,
          error: true,
          timestamp: new Date().toISOString()
        }
      }
  ...
  // Fallback broker list when database is empty
  const getFallbackBrokerList = () => {
    return [
      {
        id: 'spokeo-fallback',
        name: 'Spokeo',
        ...
        scan_priority: 1,
        is_active: true,
        api_endpoint: '/api/scrape-broker'
      },
      {
        id: 'mylife-fallback',
        name: 'MyLife',
