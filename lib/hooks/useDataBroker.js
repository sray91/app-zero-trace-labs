...
// Map Convex documents back to the snake_case row shape the pages consume.
const mapSearch = (d) =>
  d && {
    id: d._id,
    user_id: d.userId,
...
const mapRemoval = (d) =>
  d && {
    ...
    notes: d.notes,
    created_at: new Date(d._creationTime).toISOString(),
    data_sources: d.dataSource
      ? { name: d.dataSource.name, url: d.dataSource.url }
      : null,
  }
...
export const useDataBroker = () => {
  ...
  const loadSearchHistory = async () => {
    ...
    setLoading(true)
    try {
      const data = await convex.query(api.searchHistory.listForUser, {})
      setSearchHistory((data || []).map(mapSearch))
    } catch (err) {
  ...
  const loadDataSources = async () => {
    setLoading(true)
    try {
      const data = await convex.query(api.dataSources.list, {})
      setDataSources((data || []).map(mapDataSource))
    } catch (err) {
      setError(err.message)
  ...
  const loadRemovalRequests = async () => {
    ...
    setLoading(true)
    try {
      const data = await convex.query(api.removalRequests.listForUser, {})
      setRemovalRequests((data || []).map(mapRemoval))
    } catch (err) {
      setError(err.message)
  ...
  const saveSearch = async (searchData) => {
    ...
    try {
      const fullName = searchData.full_name ?? searchData.fullName
      const id = await convex.mutation(api.searchHistory.add, {
        fullName,
        phone: searchData.phone ?? undefined,
        email: searchData.email ?? undefined,
        results: searchData.results ?? undefined,
      })
      const saved = {
        id,
        user_id: user.id,
      ...
      // Update local state
      setSearchHistory(prev => [saved, ...prev])
  ...
  const submitRemovalRequest = async (requestData) => {
    ...
    try {
      ...
      const id = await convex.mutation(api.removalRequests.add, {
        dataSourceId: requestData.data_source_id ?? requestData.dataSourceId ?? undefined,
        fullName,
        phone: requestData.phone ?? undefined,
        email: requestData.email ?? undefined,
        status: requestData.status ?? undefined,
        notes: requestData.notes ?? undefined,
      })
      const saved = {
        id,
        user_id: user.id,
        data_source_id: requestData.data_source_id ?? requestData.dataSourceId ?? null,
        full_name: fullName,
      ...
      // Update local state
      setRemovalRequests(prev => [saved, ...prev])
      return { success: true, data: saved }
    } catch (err) {
      setError(err.message)
  ...
  const updateRemovalStatus = async (requestId, status, notes) => {
    try {
      await convex.mutation(api.removalRequests.updateStatus, {
        id: requestId,
        status,
  ...
  const addDataSource = async (sourceData) => {
    try {
      const id = await convex.mutation(api.dataSources.add, {
        name: sourceData.name,
        url: sourceData.url,
        apiEndpoint: sourceData.api_endpoint ?? sourceData.apiEndpoint ?? undefined,
        riskLevel: sourceData.risk_level ?? sourceData.riskLevel,
        description: sourceData.description ?? undefined,
        dataTypes: sourceData.data_types ?? sourceData.dataTypes ?? undefined,
        isActive: sourceData.is_active ?? sourceData.isActive ?? true,
      })
      const saved = mapDataSource({ _id: id, isActive: true, dataTypes: [], ...{
        name: sourceData.name,
        url: sourceData.url,
        apiEndpoint: sourceData.api_endpoint ?? sourceData.apiEndpoint,
        riskLevel: sourceData.risk_level ?? sourceData.riskLevel,
        description: sourceData.description,
        dataTypes: sourceData.data_types ?? sourceData.dataTypes ?? [],
      } })
  ...
  const initializeDataSources = async () => {
    try {
      const result = await convex.mutation(api.dataSources.seed, {})

      // Reload data sources after seeding
      await loadDataSources()
      return { success: true, message: `Added ${result.added} data sources` }
    } catch (err) {
      setError(err.message)
      return { success: false, error: err.message }
    }
